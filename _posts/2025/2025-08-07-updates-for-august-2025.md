---
title: Developer Tools, Hardware Enablement, and Multitasking Futures
description: Updates for OS 8 and Early Access
author: danrabbit
image: /images/updates-for-august-2025/desktop-blur.png
...
[Leonhard](https://github.com/leolost2605) and [Ryo](https://github.com/ryonakano) fixed a couple of issues with sidebar selections when navigating directly to a setting from search. [Ryo](https://github.com/ryonakano) fixed an issue where Sharing Settings lost its window controls when a network was not connected. And there's now an action to jump directly to the System Updates page from the context menu in the Dock or Applications Menu or via search.

# Code

Working with Git projects continues to get better thanks to [Jeremy](https://github.com/jeremypw)! There's now a new feature to clone git repositories directly from inside Code via the projects menu in the sidebar. The item for opening project folders has moved there as well, so managing your open projects now happens all in one place no matter where they come from.
...
<figure markdown="1">
![Terminal](/images/{{ page.slug }}/terminal-paste.png)
<figcaption markdown="1">
Terminal warns about more potentially dangerous commands
</figcaption>
</figure>

[Corentin](https://github.com/tintou) fixed an issue where long commands could resize windows. [Jeremy](https://github.com/jeremypw) fixed an issue where tab labels didn't properly update when using `screen` or `ssh`. And he made sure we properly close tabs when using the `exit` command.

# And More

A few small bug fixes for our Window Manager: [Corentin](https://github.com/tintou) resolved a potential crasher, [Leo](https://github.com/lenemter) improved dock hide animations, and [Leonhard](https://github.com/leolost2605) fixed an issue with revealing the panel over fullscreen apps.
...
Last month, [Ryo](https://github.com/ryonakano) made the last release of Atlas on AppCenter because we'll soon be shipping it by default in elementary OS as Maps! As part of our work to improve the experience on computers you take with you—like notebooks and tablets—we've been working on features that use your location, and shipping a Maps app is part of that work.
...
I want to give special thanks this month to [Ryan Prior](https://github.com/ryanprior) for his extremely generous one-time sponsorship! Ryan noted that this sponsorship was dedicated to the hard work of [Renato](https://l10n.elementaryos.org/user/rjltrevisan/) who has been translating elementary OS into Brazilian Portuguese. Thanks a ton for your work Renato!
